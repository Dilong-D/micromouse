...
float abs2(float x)
{
 ...
 void profiler(int32_t enkl,int32_t enkr)
 {
	 if(docelowa_V_8 >= 0){
		 ...
		 {
			 aktualna_V_8 = 0;
			 //Wylacz flage jazdy naprzod
			 moves_flags &= ~MOVES_FORWARD;
			 //moves_flags = 0;
		 }
	}else{
...
void forward(int16_t dist)
{
	...
	moves_flags |= MOVES_FORWARD;
	flaga_czuj=1;
	target_S = dist * 12.180303125;/// 0.0821;
	target_T = 0;
	...
	stan = JAZDA;
	stan_T = JAZDA;
	while(moves_flags!=0){_delay_ms(1);}
}
...
void stabilizacja_kata(void)
{
	...
	flaga_czuj=0;//flaga do stabilizowania sie wzduz scianek
	float pomfr=adcPomiar_RF()+adcPomiar_RF()+adcPomiar_RF()+1000;
	float pomfl=adcPomiar_LF()+adcPomiar_LF()+adcPomiar_LF();
	while ((abs2((pomfr-pomfl)/7000)>PI/40)){	
		pomfr=adcPomiar_RF()+adcPomiar_RF()+adcPomiar_RF()+1000;
		pomfl=adcPomiar_LF()+adcPomiar_LF()+adcPomiar_LF();
		rotateAngle((pomfr-pomfl)/7000);
	_delay_ms(20);
	}
